...
public class WindChargeModel extends HierarchicalModel<AbstractWindCharge> {
    public static final ModelLayerLocation WIND_CHARGE = new ModelLayerLocation(new ResourceLocation(FamiliarFaces.MODID, "wind_charge"), "main");
    ...
    public void setupAnim(AbstractWindCharge entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.windCharge.yRot = -ageInTicks * ROTATION_SPEED * (float) (Math.PI / 180.0);
        this.wind.yRot = ageInTicks * ROTATION_SPEED * (float) (Math.PI / 180.0);
    }
